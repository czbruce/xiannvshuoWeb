<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      用户手机号：
      <el-input v-model="listQuery.title" placeholder="用户手机号" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <!--      <el-select v-model="listQuery.importance" placeholder="Imp" clearable style="width: 90px" class="filter-item">-->
      <!--        <el-option v-for="item in importanceOptions" :key="item" :label="item" :value="item" />-->
      <!--      </el-select>-->
      <el-select v-model="listQuery.type" placeholder="所属渠道" clearable class="filter-item" style="width: 130px">
    ...
    >
      ...
      <el-table-column label="所属渠道公司" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>
      <el-table-column label="所属渠道人员" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="用户昵称" min-width="150px">
        <template slot-scope="{row}">
          ...
          <el-tag>{{ row.gettype | typeFilter }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="用户手机号" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="粉丝数" width="80px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="95px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="所属渠道" prop="author">
          <span>{{ temp.author }}</span>
        </el-form-item>
        <el-form-item label="渠道手机号" prop="author">
          <span>{{ temp.author }}</span>
        </el-form-item>
        <el-form-item label="城市" prop="author">
          <span>{{ temp.author }}</span>
        </el-form-item>
        ...
        <el-form-item label="粉丝数量" prop="title">
          <span>{{ temp.title }}</span>
        </el-form-item>
        <el-form-item label="二维码数字" prop="author">
          <span>{{ temp.author }}</span>
          <el-button>解绑二维码</el-button>
        </el-form-item>
        <el-form-item label="二维码图片" prop="title">
