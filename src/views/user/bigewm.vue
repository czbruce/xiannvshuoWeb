  ...
  <div class="app-container">
    ...
    >
      <el-table-column label="ID" prop="id" sortable="custom" align="center" width="80" class-name="">
        <template slot-scope="{row}">
          <span>{{ row.privateCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="渠道id" width="150px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="85px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item>
          <el-input v-model="temp.pagePath" label="小程序页面路径" />
        </el-form-item>
    ...
    <el-dialog v-el-drag-dialog :visible.sync="dialogTableVisible" title="二维码列表" @dragDialog="handleDrag">
...
<script>
import { bigList, addbigewm } from '@/api/ewmCode'
...
export default {
  ...
  methods: {
    getList() {
      ...
      bigList(this.listQuery).then(response => {
    ...
    handDown(e) {
      axios({
        method: 'post',
        url: process.env.VUE_APP_BASE_API + '/small-program/smallProgramQRCode/zipPrivateBarcodes',
        params: {
          batchId: e
        },
        responseType: 'blob'
